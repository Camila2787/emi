    ...
    "navigationConfig": [
        {
            ...
            priority: 100000,
            type: "link",
            url: "https://nebulae-university-reports.kb.us-central1.gcp.cloud.es.io:9243",
            target: '_blank'
        }
    ],
    "i18nLocales": {
        "en": {
            "navigation": {
                "elastic": "Elastic Reports"
            },
        },
        "es": {
            "navigation": {
                "elastic": "Reportes Elastic"
            },
        }
    }
...
class MicroFrontendIntegrator {
    ...
    initializeI18n(locale) {
        this.microFrontendListing.splice(1, 0, newNav);
    ...
    initializeNavigationConfigs() {
        ...
        const validateUserGroup = (user.roleGroups || []).includes('/PLATFORM-ADMIN');
        const validateUserRole = (user.role || []).includes('ELASTIC_REPORT_MANAGER') || (user.role || []).includes('ELASTIC_REPORT_VIEWER');
        ...
        this.navigationConfig = this.navigationConfig.filter(nav => nav.id === 'elastic' ? ( validateUserGroup || validateUserRole) : true );
        this.navigationConfig = this.navigationConfig.filter(nav => (nav.type === 'item') || (nav.type === 'link') || ((nav.children || []).length > 0))
    }
