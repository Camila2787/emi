...
class KeycloakService {
    ...
    constructor() {
        ...
        this.tokenCheckerTimer = setInterval(() => {
            if (this.user) {
                this.updateToken();
            }
        }, 30000);
    ...
    onKeycloakEvent = (event, error) => {
        switch (event) {
            case "onReady":
                //console.log('keycloakService:', 'onKeycloakEvent.onReady ', Date.now());
                if (this.onReady) {
                    this.onReady();
                }
                break;
            ...
            case "onAuthLogout": console.log('keycloakService:', 'onKeycloakEvent.onAuthLogout', error);
                ...
                break;
            case "onTokenExpired":
                //console.log('keycloakService:', 'onKeycloakEvent.onTokenExpired', error, Date.now());
    ...
    updateToken() {
        this.keycloak.updateToken(KEYCLOAK_CONFIG.minValidityToken).then(function () {
        }).catch(function () {
    ...
    isAuthTokenValid = () => {
        ...
        else if (decoded.realm_access.roles.includes('HEADLESS')) {
            return false;
        }
        else {
            return true;
    ...
    refreshUser() {
        ...
        try {
            this.user.data.companyIds=decodedToken.companyIds ? decodedToken.companyIds : null;
        } catch (error) {
            console.log(`KeycloakService:refreshUser: ERROR GETTING COMPANYIDS: ${decodedToken.companyIds}, IS ARRAY: ${ Array.isArray(decodedToken.companyIds) }`);
            this.user.data.companyIds=[decodedToken.companyId];
        }
    }
    ...
    logout = () => {
        ...
        clearInterval(this.tokenCheckerTimer)
    }
