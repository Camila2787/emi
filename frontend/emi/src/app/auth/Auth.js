...
class Auth extends Component {
    ...
    keycloakCheck = () => new Promise((resolve,reject) => {
        //const keycloakService = new KeycloakService();

        keycloakService.onReady = () => {
 
            
            if (keycloakService.user.role.includes("HEADLESS")) {
                this.props.showMessage({ message: 'Undefined or non valid JWT Token' });
                const msg = 'HEADLESS ACCOUNT - not suitable for EMI';
                console.log(msg);
                reject();
            }
            if (keycloakService.user.role.includes("ENDUSER")) {
                this.props.showMessage({ message: 'Undefined or non valid JWT Token' });
                const msg = 'END_USER ACCOUNT - not suitable for EMI';
                console.log(msg);
                reject();
            }
